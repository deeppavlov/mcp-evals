...
@dataclass
class QualifiedStudentsFileExists(Evaluator["EnglishTalentTask", AgentRunResult]):
    """Evaluator that checks qualified_students.txt file exists."""
    ...
    async def evaluate(self, ctx: EvaluatorContext["EnglishTalentTask", AgentRunResult]) -> EvaluatorOutput:
        ...
        if not answer_file.exists():
            return EvaluationReason(value=0.0, reason="File 'qualified_students.txt' not found")
...
class FileFormat(Evaluator["EnglishTalentTask", AgentRunResult]):
    ...
    async def evaluate(self, ctx: EvaluatorContext["EnglishTalentTask", AgentRunResult]) -> EvaluatorOutput:  # noqa: PLR0911
        ...
        for student in students:
            ...
            if student["id"] != expected["id"]:
                return EvaluationReason(
                    value=0.0,
                    reason=(
                        f"ID mismatch for {student['name']}: "
                        f"expected {expected['id']}, got {student['id']}"
                    ),
                )
            if student["email"] != expected["email"]:
                return EvaluationReason(
                    value=0.0,
                    reason=(
                        f"Email mismatch for {student['name']}: "
                        f"expected {expected['email']}, got {student['email']}"
                    ),
                )
